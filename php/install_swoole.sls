...
extract_install_swoole:
  cmd.run:
    ...
    - cwd: /usr/local/src
    - name: tar zxvf swoole-1.9.20.tgz && cd swoole-1.9.20  &&  /opt/php/bin/phpize && ./configure --with-php-config=/opt/php/bin/php-config && make && make install && "extension=swoole.so" >> /opt/php/etc/php.ini 
    - require:
       - file: swoole_source
